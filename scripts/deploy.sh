...
SSH_HOST="ssh-andreas-jaeggi.nona.company"
...
KESTRA_URL=https://kestra-andreas-jaeggi.nona.company
EOF
...
CB_SERVER_URL=https://cloudbeaver-andreas-jaeggi.nona.company
...
REDIRECT_URL=https://hoppscotch-andreas-jaeggi.nona.company
WHITELISTED_ORIGINS=https://hoppscotch-andreas-jaeggi.nona.company
VITE_BASE_URL=https://hoppscotch-andreas-jaeggi.nona.company
VITE_SHORTCODE_BASE_URL=https://hoppscotch-andreas-jaeggi.nona.company
VITE_ADMIN_URL=https://hoppscotch-andreas-jaeggi.nona.company/admin
VITE_BACKEND_GQL_URL=https://hoppscotch-andreas-jaeggi.nona.company/backend/graphql
VITE_BACKEND_WS_URL=wss://hoppscotch-andreas-jaeggi.nona.company/backend/graphql
VITE_BACKEND_API_URL=https://hoppscotch-andreas-jaeggi.nona.company/backend/v1
...
PREFECT_UI_API_URL=https://prefect-andreas-jaeggi.nona.company/api
EOF
...
LAKEFS_GATEWAYS_S3_DOMAIN_NAME=s3.lakefs-andreas-jaeggi.nona.company
...
APPSMITH_CUSTOM_DOMAIN=https://appsmith-andreas-jaeggi.nona.company
EOF
...
NC_PUBLIC_URL=https://nocodb-andreas-jaeggi.nona.company
...
EOF
    ...
    if [ -n "$INFISICAL_TOKEN" ] && [ -n "$PROJECT_ID" ]; then
        ...
        build_folder "redpanda" \
            ...
            "REDPANDA_KAFKA_PUBLIC_URL" "redpanda-kafka-andreas-jaeggi.nona.company:9092" \
            "REDPANDA_SCHEMA_REGISTRY_PUBLIC_URL" "redpanda-schema-registry-andreas-jaeggi.nona.company:18081" \
            "REDPANDA_ADMIN_PUBLIC_URL" "redpanda-admin-andreas-jaeggi.nona.company:9644" \
            "REDPANDA_CONNECT_PUBLIC_URL" "redpanda-connect-api-andreas-jaeggi.nona.company:4195"
        ...
        build_folder "gitea" \
            ...
            "GITEA_REPO_URL" "https://git-andreas-jaeggi.nona.company/${GITEA_REPO_OWNER:-$ADMIN_USERNAME}/${REPO_NAME:-nexus-${DOMAIN//./-}-gitea}.git" \
            "GITEA_DB_PASSWORD" "$GITEA_DB_PASS"
    ...
    cron: \"*/15 * * * *\"'" >/dev/null 2>&1 || true
            ...
            if echo "$ENABLED_SERVICES" | grep -qw "woodpecker" && [ -n "$WOODPECKER_AGENT_SECRET" ]; then
                ...
                OAUTH_RESULT=$(ssh nexus "curl -s -X POST 'http://localhost:3200/api/v1/user/applications/oauth2' \
                    ...
                    -d '{
                        \"name\": \"Woodpecker CI\",
                        \"redirect_uris\": [\"https://woodpecker-andreas-jaeggi.nona.company/authorize\"],
...
if echo "$ENABLED_SERVICES" | grep -qw "wikijs" && [ -n "$WIKIJS_ADMIN_PASS" ]; then
    (
        ...
        SETUP_PAYLOAD=$(jq -n \
            ...
            --arg url "https://wiki-andreas-jaeggi.nona.company" \
